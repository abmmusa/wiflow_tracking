# Usage: python wifimatcher_run <input file>
...
#m = WiFiMatcher("../mapinference/trr12/roosevelt_small.osmdb")  #is it a good practice?

class WiFiMatcherRun:
    def __init__(self):
        self.m = WiFiMatcher("../mapinference/trr12/roosevelt_small.osmdb")
    ...
    def run_viterbi(self, fname):
        ...
        for i in range(1, len(data)):
            ...
            V,p = self.m.step( obslast, V, p )

            timegap = int(timenext)-int(timelast)
            if(timegap>1):

                if timegap > MAX_TIMEGAP_BEFORE_NEW_VITERBI_COMPUTAION:
                    ...
                    for i in range(1, timegap):
                        V,p=self.m.step( noobs, V, p )



                ############################                                                                                                                    
                ## Simplified version for no obs, doesn't consider how much timegap there is                                  
                ###########################                                                                                                                
                #for i in range(1, timegap):
    ...
    #    self.run_viterbi(input_file)
                    
if __name__ == '__main__':
    matcher = WiFiMatcherRun()
    matcher.run_viterbi(input_file)
