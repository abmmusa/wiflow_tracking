...
class Edge:
    ...
    def __str__(self):
        return "(%s,%s)"%(self.in_node,self.out_node)

    @property
    def length(self):
        return spatialfunclib.distance4(self.in_node.latitude, self.in_node.longitude,self.out_node.latitude, self.out_node.longitude)
    ...
    # point is (latitude, longitude)
    def distance_to(self, point):
        return spatialfunclib.distance_to_segment(((self.in_node.latitude,self.in_node.longitude),(self.out_node.latitude,self.out_node.longitude)), (point[0],point[1]))
